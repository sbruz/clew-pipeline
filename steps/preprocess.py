...
def split_paragraphs_with_openai(text, lang="en", max_chars=4000) -> str:
    ...
    system_prompt = (
        ...
        "- Каждый абзац должен быть до 140 символов с пробелами.\n"
...
def simplify_text_for_beginners(book_id: int, lang: str, max_chars: int):
    ...
    system_prompt = (
        "Ты — дружелюбный рассказчик, который помогает адаптировать главы книг для изучающих английский.\n"
        "Правила:\n"
        "- Перепиши текст на понятном, современном английском языке уровня A2–B1.\n"
        "- Сохраняй разбивку на абзацы — не объединяй и не пропускай абзацы.\n"
        "- Пиши на английском языке.\n"
        "- Используй короткие, простые и естественные предложения с хорошим «потоком».\n"
        "- Каждый абзац должен передавать тот же смысл, но быть более живым и понятным. \n"
        "- Не используй старомодных или книжных слов. Пиши современно, легко и по-человечески.\n"
        "Верни только один объект главы по схеме ChapterItem. Без пояснений."
    )
...
def split_into_sentences(book_id, lang="en", max_chars=4000):
    ...
    system_prompt = (
        ...
        "Твоя задача — разбить длинные предложения из 20 и более слов на короткие, сохранив логическую и грамматическую корректность.\n"
        "- Оставь без изменений предложения, которые при разбивке могут потерять смысл или стать грамматически неверными.\n"
